...
class Configuration implements ConfigurationInterface
{
    ...
    public function addConfigurationSection(ArrayNodeDefinition $node)
    {
        $node
            ->addDefaultsIfNotSet()
            ->children()
                ->scalarNode('website_name')->cannotBeEmpty()->defaultValue('Qcm Demo')->end()
                ->arrayNode('configuration')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('max_questions')->cannotBeEmpty()->defaultValue(40)->end()
                        ->arrayNode('question_level')
                        ...
                        ->scalarNode('time_per_question')->cannotBeEmpty()->defaultValue(60)->end()
                    ->end()
                ->end()
            ->end();
    }
                    ...
                    ->addDefaultsIfNotSet()
                        ->children()
                            ...
                            ->arrayNode('statistics')
                                ->addDefaultsIfNotSet()
                                ->children()
                                    ->scalarNode('class')->defaultValue('Qcm\Bundle\CoreBundle\Statistics\QuestionnaireStatistics')->end()
                                    ->scalarNode('template')->defaultValue('Qcm\Bundle\CoreBundle\Statistics\Answers\Template')->end()
                                ->end()
                            ->end()
                        ->end()
                    ->end()
                    ...
                    ->children()
                        ...
                        ->arrayNode('user')
                            ...
                            ->children()
                                ...
                                ->scalarNode('repository')->defaultValue('Qcm\Bundle\CoreBundle\Doctrine\ORM\UserRepository')->end()
                        ...
                        ->arrayNode('user_session')
                            ...
                            ->children()
                                ...
                                ->scalarNode('repository')->defaultValue('Qcm\Bundle\CoreBundle\Doctrine\ORM\UserSessionRepository')->end()
                                ->scalarNode('form')->defaultValue('Qcm\Bundle\CoreBundle\Form\Type\UserSessionFormType')->end()
                            ->end()
                        ->end()
                        ->arrayNode('user_session_configuration')
                        ->addDefaultsIfNotSet()
                            ->children()
                                ->scalarNode('model')->end()
                                ->scalarNode('form')->defaultValue('Qcm\Bundle\CoreBundle\Form\Type\UserSessionConfigurationFormType')->end()
                            ->end()
                        ->end()
                        ->arrayNode('category')
                            ->addDefaultsIfNotSet()
                            ->children()
                                ...
                                ->scalarNode('repository')->defaultValue('Qcm\Bundle\CoreBundle\Doctrine\ORM\CategoryRepository')->end()
